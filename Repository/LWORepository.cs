    ...
    public class LWORepository : ILWORepository
    {
        ...
        public async Task<LWOModel> GetById(Guid id)
        {
            using (var connection = new SqlConnection(_connectionString))
            {
                ...
                var command = new SqlCommand(@"SELECT * FROM LWO WHERE ID = @ID", connection);
        ...
        public async Task Create(LWOModel lwo)
        {
            using (var connection = new SqlConnection(_connectionString))
            {
                ...
                var command = new SqlCommand(@"
                    ...
                    VALUES (@ID, @WONumber, @WODate, @WOType, @Activity, @HM, @TimeStart, @TimeEnd, @PIC, @LWOType, @Version)", connection);
